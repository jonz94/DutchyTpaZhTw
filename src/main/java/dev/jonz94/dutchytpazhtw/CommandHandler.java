      ...
      if (targetMap.containsValue(senderP.getUniqueId())) {
        for (Map.Entry<UUID, UUID> entry : targetMap.entrySet()) {
          if (((UUID) entry.getValue()).equals(senderP.getUniqueId())) {
            ...
            sender.sendMessage(
              ChatColor.GOLD + "你接受了 " + ChatColor.RED + tpRequester.getDisplayName() + ChatColor.GOLD + " 的傳送請求"
            );
      ...
      if (targetMap.containsValue(senderP.getUniqueId())) {
        for (Map.Entry<UUID, UUID> entry : targetMap.entrySet()) {
          if (((UUID) entry.getValue()).equals(senderP.getUniqueId())) {
            ...
            sender.sendMessage(
              ChatColor.GOLD + "你已拒絕了 " + ChatColor.RED + originalSender.getDisplayName() + ChatColor.GOLD + " 的傳送請求"
            );
